...
router.post("/cart/add", async(req, res) => {
  try{
      ...
      if (existingItem.rowCount == 0){
        const cartitem = await pool.query(`INSERT INTO cartitem (userid, productid, quantity, size, itemtotal) VALUES (${userId}, ${productid}, ${quantity}, '${size}', ${itemtotal}) RETURNING *`)
        res.json(cartitem.rows)
      //if exists update
      }else{
        const cartitem = await pool.query(`UPDATE cartitem SET quantity=${quantity}, itemtotal=${itemtotal} WHERE  userid=${userId} and productid=${productid} RETURNING *`)
        res.json(cartitem.rows)
      } 
    } catch(err){
